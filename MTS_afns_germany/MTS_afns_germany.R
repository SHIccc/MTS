...
# ------------------------------------------------------------------------------
# Quantlet:    MTS_afns_germany
# ------------------------------------------------------------------------------
# Description: The estimation results for Germany derived from the AFNS model in
...
yieldadj_joint = function(sigma11, sigma12 = 0, sigma13 = 0, sigma21 = 0, sigma22, sigma23 = 0, 
                          sigma31 = 0, sigma32 = 0, sigma33, sigma44, lambda, time = maturity) {
...
Meloading_joint = function(lambda, alphaS, alphaC, time = maturity) {
  row1 = c(1, (1 - exp(-lambda * time))/(lambda * time), 
           (1 - exp(-lambda * time))/(lambda * time) - exp(-lambda * time), 0)
  row2 = c(0, alphaS * ((1 - exp(-lambda * time))/(lambda * time)), 
            alphaC * ((1 - exp(-lambda * time))/(lambda * time) - exp(-lambda * time)), 1)
...
afnsss = function(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, 
  ...
  Tt = matrix(c(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, 
                 t14, t15, t16), nr = 4)
  Zt = rbind(Meloading_joint(lambda = l1, alphaS = g1, alphaC = g2, time = demat[1]), 
              Meloading_joint(lambda = l1, alphaS = g1, alphaC = g2, time = demat[2]), 
              Meloading_joint(lambda = l1, alphaS = g1, alphaC = g2, time = demat[3]))
  ct = matrix(rep(c(yieldadj_joint(sigma11 = h1, sigma22 = h2, sigma33 = h3, 
                                    sigma44 = h4, lambda = l1, time = demat[1]), 
                     yieldadj_joint(sigma11 = h1, sigma22 = h2, sigma33 = h3, 
                                    sigma44 = h4, lambda = l1, time = demat[2]), 
                     yieldadj_joint(sigma11 = h1, sigma22 = h2, sigma33 = h3, sigma44 = h4, 
                                    lambda = l1, time = demat[1])), each = 2), nr = 6, nc = 1)
...
theta = c(t = c(0.7, 0, 0, 0, 0, 0.7, 0, 0, 0, 0, 0.7, 0, 0, 0, 0, 0.7), 
          s = c(0.3, 0.3, 0.3, 0.3), g = c(0.6, 0.6), l1 = c(0.7), 
          h = c(0.2, 0.2, 0.2, 0.2))
...
matrix(rowMeans(ans$vt[, 2:67]), nr = 6)
joide0915ans = ans
joide0915fit = fit
save(joide0915ans, file = "joide0915ans.RData")
save(joide0915fit, file = "joide0915fit.RData")

## The plots of filtered and predicted state variables Another approach:
## plot.fkf(ans, CI=NA)
plot(ans$at[1, -1], type = "l", col = "red", ylab = "State variables", 
     xlab = "", ylim = c(-8, 6), lwd = 2)
