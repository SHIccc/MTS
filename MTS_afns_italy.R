...
# ------------------------------------------------------------------------------
# Quantlet:    MTS_afns_italy
# ------------------------------------------------------------------------------
# Description: The estimation results for Italy derived from the AFNS model in
...
yieldadj_joint <- function(sigma11,sigma12=0,sigma13=0,sigma21=0,sigma22,sigma23=0,sigma31=0,sigma32=0,sigma33,sigma44,lambda, time=maturity)
{
  ...
  adj2<-Btilde*(1/(2*lambda^2)-(1-exp(-lambda*time))/(lambda^3*time)+(1-exp(-2*time*lambda))/(3*lambda^3*time))
  adj3<-Ctilde*(1/(2*lambda^2)+exp(-lambda*time)/(lambda^2)-time*exp(-2*lambda*time)/(4*lambda)-3*exp(-2*lambda*time)/(4*lambda^2)-2*(1-exp(-lambda*time))/(lambda^3*time)+5*(1-exp(-2*lambda*time))/(8*lambda^3*time))
  adj4<-Dtilde*(time/(2*lambda)+exp(-lambda*time)/(lambda^2)-(1-exp(-lambda*time))/(lambda^3*time))
  adj5<-Etilde*(3*exp(-lambda*time)/(lambda^2)+time/(2*lambda)+time*exp(-lambda*time)/lambda-3*(1-exp(-lambda*time))/(lambda^3*time))
  adj6<-Ftilde*(1/(lambda^2)+exp(-lambda*time)/(lambda^2)-exp(-2*lambda*time)/(2*lambda^2)-3*(1-exp(-lambda*time))/(lambda^3*time)+3*(1-exp(-2*lambda*time))/(4*lambda^3*time))
...
{
  row1 <- c(1,(1-exp(-lambda*time))/(lambda*time),(1-exp(-lambda*time))/(lambda*time)-exp(-lambda*time),0)
  row2 <- c(0,alphaS*((1-exp(-lambda*time))/(lambda*time)),alphaC*((1-exp(-lambda*time))/(lambda*time)-exp(-lambda*time)),1)
  MatrixB <- rbind(row1,row2)
   return(MatrixB)
}

afnsss <- function(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16,s1,s2,s3,s4,g1,g2,l1,h1,h2,h3,h4){
  Tt <- matrix(c(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16),nr=4)
  Zt <- rbind(Meloading_joint(lambda=l1,alphaS=g1,alphaC=g2, time=itmat[1]),Meloading_joint(lambda=l1,alphaS=g1,alphaC=g2, time=itmat[2]),Meloading_joint(lambda=l1,alphaS=g1,alphaC=g2, time=itmat[3]))
  ct <- matrix(rep(c(yieldadj_joint(sigma11=h1, sigma22=h2, sigma33=h3, sigma44=h4, lambda=l1, time=itmat[1]),yieldadj_joint(sigma11=h1, sigma22=h2, sigma33=h3, sigma44=h4, lambda=l1, time=itmat[2]),yieldadj_joint(sigma11=h1, sigma22=h2, sigma33=h3, sigma44=h4, lambda=l1, time=itmat[1])),each=2), nr=6, nc=1)
  dt <- matrix(c(1-t1,t2,t3,t4,t5,1-t6,t7,t8,t9,t10,1-t11,t12,t13,t14,t15,1-t16),nr=4)%*%matrix(c(s1,s2,s3,s4),nr=4)
...
objective <- function(theta,yt){
  sp <- afnsss(theta["t1"],theta["t2"],theta["t3"],theta["t4"],theta["t5"],theta["t6"],theta["t7"],theta["t8"],theta["t9"],theta["t10"],theta["t11"],theta["t12"],theta["t13"],theta["t14"],theta["t15"],theta["t16"],theta["s1"],theta["s2"],theta["s3"],theta["s4"],theta["g1"],theta["g2"],theta["l1"],theta["h1"],theta["h2"],theta["h3"],theta["h4"])
  ans <- fkf(a0=sp$a0, P0=sp$P0, dt=sp$dt, ct=sp$ct, Tt=sp$Tt, Zt=sp$Zt, HHt=sp$HHt, GGt=sp$GGt, yt=yt)  
  return(-ans$logLik)
}

theta <- c(t=c(0.7,0,0,0,0,0.7,0,0,0,0,0.7,0,0,0,0,0.7), s = c(0.3,0.3,0.3,0.3), g = c(0.6,0.6), l1 = c(0.6), h = c(0.2, 0.2, 0.2, 0.2))
...
plot(ans$vt[1,],type="l", ylim=c(-2,4))
lines(ans$vt[2,])
lines(ans$vt[3,])
lines(ans$vt[4,])
lines(ans$vt[5,])
